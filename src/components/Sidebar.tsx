...
type SidebarRoute = 'chat' | 'profile' | 'search' | 'agents' | 'conversations' | 'designagent' | 'promptcoach' | 'timeentry';
...
const Sidebar: React.FC<SidebarProps> = ({ className = '', isOpen, isCollapsed = false, onToggle, onCollapse, onNavigate }) => {
  ...
  const handleNavigate = (route: SidebarRoute) => {
    console.log('[Sidebar] handleNavigate called with route:', route);
    if (onNavigate) {
      console.log('[Sidebar] onNavigate called with route:', route);
      onNavigate(route);
    }
    if (window.matchMedia('(max-width: 640px)').matches && onCollapse) {
      console.log('[Sidebar] onCollapse called (mobile)');
      onCollapse();
    }
  };
        ...
        <nav className={`flex-1 ${isCollapsed ? 'p-1' : 'p-2'}`}>
          <div className="space-y-1">
            ...
            <div className="mt-4">
              ...
              {agentsExpanded && (
                <div className="ml-6 mt-1 space-y-1">
                  ...
                  <button className="w-full flex items-center space-x-3 px-3 py-2 text-left text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-md transition-colors" onClick={() => {
                    if (onNavigate) onNavigate('promptcoach');
                    if (onCollapse) onCollapse();
                  }}>
                  ...
                  <button className="w-full flex items-center space-x-3 px-3 py-2 text-left text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-md transition-colors" onClick={() => {
                    if (onNavigate) onNavigate('timeentry');
                    if (onCollapse) onCollapse();
                  }}> 
                    <div className="w-6 h-6 bg-purple-100 rounded-full flex items-center justify-center">
            ...
            <button
              className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} px-3 py-2 text-left text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-md transition-colors`}
              onClick={() => {
                if (onNavigate) onNavigate('designagent');
                if (onCollapse) onCollapse();
              }}
            >
              <Plus size={16} />
              {!isCollapsed && <span className="text-sm font-medium">Design New Agent</span>}
            </button>
